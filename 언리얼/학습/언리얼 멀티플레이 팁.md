...
> [!summary] 요약
> 언리얼 멀티플레이 팁은 서버 권한(Authority), ownership, replication, RPC, relevancy를 섞어 쓸 때 자주 생기는 실수를 정리한 네트워킹 점검 노트다.
> 멀티플레이 기능이 "서버에서는 되는데 클라이언트에서 안 되는" 상황을 추적할 때 먼저 본다.
> 핵심은 Game Framework 객체의 네트워크 역할, 호출 주체, 복제(Replication) 조건, 초기화 타이밍을 분리해서 확인하는 것이다.

## 핵심 결론

- `HasAuthority()`, owner, net mode, net role은 서로 다른 질문에 답하므로 같은 의미로 쓰지 않는다.
- 상태는 replication으로 남기고, 순간 요청이나 연출은 RPC로 처리하는 식으로 책임을 나눈다.
- 클라이언트 문제가 보이면 actor replication 설정, owner connection, relevancy, 호출 시점을 순서대로 확인한다.

## 개요

이 문서는 WizardCell의 멀티플레이 팁 글을 학습용으로 수집한 자료다. 원문 전체를 그대로 외우기보다, 아래 항목을 네트워크 디버깅 체크리스트처럼 훑는 용도로 사용한다.

## 왜 필요한가

네트워크 버그는 기능 코드가 틀려서보다 실행 위치와 복제 조건을 잘못 가정해서 생기는 경우가 많다. 언리얼 멀티플레이 팁을 볼 때는 "서버의 사실", "클라이언트의 요청", "복제로 전달되는 상태"를 분리해야 한다.
...
- 원문: [Unreal Engine Multiplayer Tips and Tricks](https://wizardcell.com/unreal/multiplayer-tips-and-tricks/)
- 발행: 2023-03-19
- 갱신: 2023-08-13

## 빠른 활용법

1. 문제 상황이 상태 동기화 문제인지 원격 함수 호출 문제인지 먼저 나눈다.
2. 관련 actor가 replicated인지, owner connection이 있는지 확인한다.
3. RPC라면 호출 위치와 실행 대상을 확인한다.
4. replication이라면 relevancy, dormancy, update frequency, RepNotify 타이밍을 확인한다.
5. 원문 항목은 필요한 주제만 찾아 읽고, 실제 프로젝트에서는 엔진 로그와 네트워크 프로파일러로 검증한다.
