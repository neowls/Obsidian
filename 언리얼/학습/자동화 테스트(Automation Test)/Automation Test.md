...
> [!summary] 요약
> Automation Test는 런타임 기능을 관찰, 조정, 검증하기 위한 언리얼 도구/서브시스템(Subsystem) 주제다.
...
관찰 도구와 런타임 기능은 "보이는 결과"만으로 원인을 찾기 어렵다. Automation Test를 볼 때는 입력 조건, 설정, 실제 frame/runtime 출력을 같은 표로 맞춰야 한다.
...
| Runtime Test | 런타임 모듈(Module)에서 실행 | 엔진/게임 로직 회귀 |
...
| 별도 test module | 의존성과 빌드(Build) 대상을 분리하기 좋다 | `.Build.cs`, `.uproject` 설정이 늘어난다 |
...
누군가 row 이름을 바꾸거나 데이터 테이블을 옮기면, 패키징(Packaging) 전에 바로 드러난다.
...
- [ ] 관련 plugin과 project setting이 활성화되어 있다.
- [ ] runtime subsystem/component가 생성되고 tick 또는 capture가 실행된다.
- [ ] 로그, stat, profiler, visualizer 중 하나 이상으로 상태를 확인했다.
- [ ] PIE와 packaged build의 platform/config 차이를 확인했다.
- [ ] 반복 검증이 필요한 항목은 automation test 또는 명시적 재현 절차로 남겼다.
