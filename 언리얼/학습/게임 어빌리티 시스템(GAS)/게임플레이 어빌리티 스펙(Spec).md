...
# 게임플레이 어빌리티(Gameplay Ability) 스펙(Spec)

> [!summary] 요약
> 게임플레이 어빌리티 스펙(Gameplay Ability Spec)은 ASC가 보유한 ability 인스턴스, 입력 ID, 레벨, 핸들을 묶어 관리하는 실행 단위다.
> ability를 부여한 뒤 실제 활성화, 입력 처리, 복제(Replication) 상태를 추적할 때 확인한다.
> 핵심은 ability class 자체가 아니라 ASC 안의 spec handle과 spec container가 런타임 상태를 들고 있다는 점이다.

## 핵심 결론

- `GiveAbility()`는 ability class를 직접 실행하는 것이 아니라 ASC의 activatable ability spec 목록에 항목을 추가한다.
- 입력, 레벨, source object, dynamic tag 같은 런타임 정보는 spec을 통해 추적한다.
- 활성화가 실패하면 ability class보다 spec handle, actor info, 입력 바인딩, activation 조건을 먼저 확인한다.
...
GAS 문제는 단일 클래스 하나보다 여러 객체의 책임 경계가 어긋날 때 발생한다. 게임플레이 어빌리티 스펙(Spec)을 볼 때는 "누가 상태를 소유하는가", "어느 시점에 서버와 클라이언트가 합의하는가", "실패했을 때 어디서 되돌아가는가"를 먼저 분리해야 한다.
